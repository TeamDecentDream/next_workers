...
const OnWork: FC = () => {
  ...
  const handleWorkState = () => {
    if (auth.workState) {
      axios
        .post(
          GinServerBaseURL + "/attendance/leave",
          {},
      ...
      axios
        .post(
          GinServerBaseURL + "/attendance/enter",
          {},
          { headers: { Authorization: auth.accessToken } }
        )
        .then((resp) => {
  ...
  return (
    <div
      className={`w-20 h-8 pt-[4px] text-sm text-center font-semibold text-${color} border-2 border-${color} rounded-lg cursor-pointer`}
